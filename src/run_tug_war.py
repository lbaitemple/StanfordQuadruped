        ...
        while True:
            ...
            if back_touched and (current_time - last_trigger_time > DEBOUNCE_TIME):
                if not tug_active:
                    ...
                    print("[BACK] Continuing pull (full strength)")
                    Move.MovementLib.clear()
                    Move.tug_of_war(pulling=True, pull_strength=1.0)
                    movementCtl = MovementScheme(Move.MovementLib)
            ...
            # Front sensor - stop tug-of-war
            if front_touched and (current_time - last_trigger_time > DEBOUNCE_TIME):
                print("[FRONT] Stopping tug-of-war")
                Move.MovementLib.clear()
                Move.tug_of_war_stop()
                movementCtl = MovementScheme(Move.MovementLib)
                tug_active = False
                last_trigger_time = current_time

            # Left sensor - reduce strength
            if left_touched and tug_active and (current_time - last_trigger_time > DEBOUNCE_TIME):
            ...
            if right_touched and tug_active and (current_time - last_trigger_time > DEBOUNCE_TIME):
            ...
            # Debug: print velocity every second
            if tug_active and int(current_time) != int(current_time - config.dt):
                print(f"[DEBUG] velocity: {command.horizontal_velocity}, pitch: {command.pitch}")
